...
- 存储为 `profiles.json`(便携式:exe 目录可写则存 exe 目录,否则回退 `%AppData%\EasyWin`)
...
### 🖥 远程桌面
保存 mstsc 连接(服务器/用户/分辨率/管理模式),一键发起远程控制;勾选"记住凭据"后通过 Windows 凭据管理器免密登录,密码经 DPAPI 加密存储。
...
```
src/EasyWin/
├── Models/      NetworkAdapterInfo、IpProfile、LaunchItem 等
...
├── Views/       FluentWindow 主窗体 + 7 个页面 + 方案编辑窗口
